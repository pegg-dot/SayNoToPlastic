...
async function configuredSiteUrl() {
  try {
    ...
    return typeof env.PUBLIC_SITE_URL === "string" ? env.PUBLIC_SITE_URL : undefined;
  } catch {
...
export async function POST(request: Request) {
  try {
    ...
    if (existingSubscriber?.status === "active") {
      if (program === "learning-series") {
        ...
        if (existingSubscriber.providerStatus !== "synced") {
          await db.update(emailOutbox).set({
            status: "dead",
            updatedAt: now,
            lastError: "welcome_replaced_after_token_rotation",
            nextAttemptAt: null,
          }).where(and(
            eq(emailOutbox.kind, "welcome"),
            eq(emailOutbox.recipient, email),
            inArray(emailOutbox.status, ["pending", "queued", "failed"]),
          ));
          const [replacementWelcome] = await db.insert(emailOutbox).values({
            kind: "welcome",
            recipient: email,
            status: "pending",
            payload: JSON.stringify({ firstName, source, unsubscribeToken, siteOrigin }),
            updatedAt: now,
      ...
      return Response.json({ ok: true, delivery: existingSubscriber.providerStatus === "synced" ? "sent" : "queued" }, { status: 200 });
    }
    ...
    const series = program === "learning-series" ? await enrollInLearningSeries({ email, firstName, unsubscribeToken, siteOrigin }) : null;
    return Response.json({ ok: true, delivery: delivery === "sent" ? "sent" : "queued", ...(series ? { series } : {}) }, { status: 201 });
  } catch (error) {
