...
function PendingMedia({ id }: { id: string }) {
  ...
  return <figure className="media-pending-card"><img src={entry.thumbnail} width="1536" height="1024" loading="lazy" alt={entry.thumbnailAlt} /><figcaption><span>{entry.platform}</span><strong>Official link pending owner confirmation</strong><p>{entry.description}</p><small>Playback remains disabled until the URL, publication date, thumbnail rights, and owner approval are recorded in the media registry.</small></figcaption></figure>;
}
...
export default function MediaPage() {
  ...
  return <><Header /><main id="main-content" tabIndex={-1} className={`media-page ${styles.phase11}`}>
    ...
    <section id="featured-talk" className="media-feature ivory"><div className="media-feature-copy"><p className="eyebrow dark">TEDxMiami · temporary recording</p><h2>{tedx?.title ?? "The Invisible Inheritance of Nanoplastics"}</h2><p>A temporary audience-recorded YouTube video is available now. It is not the official TEDx release and will be replaced as soon as the official public video arrives.</p><dl><div><dt>Speaker</dt><dd>Elie R. Haddad, MD</dd></div><div><dt>Platform</dt><dd>{tedx?.platform ?? "TEDxMiami"}</dd></div><div><dt>Status</dt><dd>Temporary audience recording</dd></div></dl></div><PendingMedia id="tedx-invisible-inheritance" /></section>
