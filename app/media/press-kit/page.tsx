...
export default function PressKitPage(){return <><Header/><main id="main-content" tabIndex={-1} className="inner-page press-kit-page">
  <section className="press-kit-hero"><div><p className="eyebrow">Press kit</p><h1>Context before the conversation.</h1><p>Use the current web biography, project facts, interview topics, and available assets below. High-resolution press files remain pending owner approval.</p></div><aside><span>Media contact</span><strong>{SUPPORT_EMAIL}</strong><p>Include the outlet, audience, format, deadline, publication date, and central question.</p><a href="/contact">Submit an inquiry →</a></aside></section>
  <section className="press-kit-bios ivory"><div><p className="eyebrow dark">Current biography</p><h2>Short and extended versions.</h2></div><div><article><span>Short bio</span><p>{MEDIA_KIT.shortBio}</p></article><article><span>Extended bio</span><p>{MEDIA_KIT.longBio}</p></article></div></section>
