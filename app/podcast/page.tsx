...
export default async function PodcastPage() {
  const [platforms, ownerCopy] = await Promise.all([
    getEffectivePodcastPlatforms(),
    getAdminContentValues(["podcast.series_label", "podcast.series_heading", "podcast.series_body"]),
  ]);
  const seriesLabel = ownerCopy["podcast.series_label"] || "The Plastic Age";
  const seriesHeading = ownerCopy["podcast.series_heading"] || "The journey begins with plastic. It does not end there.";
  const seriesBody = ownerCopy["podcast.series_body"] || "Our first series, The Plastic Age, follows the remarkable story of how plastic transformed our world, entered our bodies, and became one of the defining challenges of our time.";
  ...
  return <><Header /><main id="main-content" tabIndex={-1} className={styles.page}>
    ...
    <section className={styles.story}><div className={styles.storyIndex}><span>01</span><p>Current series</p></div><div className={styles.storyCopy}><p className={styles.eyebrow}>{seriesLabel}</p><h2>{seriesHeading}</h2><p>{seriesBody}</p><p>Future series will venture beyond plastic into other questions of health, wellness, the environment and what it means to live consciously in a rapidly changing world.</p></div><aside className={styles.storyAside}><strong>Series, not episode feed</strong><p>This site introduces the larger podcast. Individual episodes remain on the listening platforms, where each series can be browsed in full.</p></aside></section>
