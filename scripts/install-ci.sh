...
# Local macOS development uses npm's lockfile/integrity guarantees directly.
# The more defensive flock, /proc, and seeded-cache path below remains the
# authoritative Sites/Linux installer.
if [[ "$(uname -s)" == "Darwin" ]]; then
  ...
  if ! mkdir "${lock_dir}" 2>/dev/null; then
    echo "Another dependency install appears to be running for ${SITES_PROJECT_ROOT}." >&2
    exit 75
  fi
  cleanup_lock() { rmdir "${lock_dir}" 2>/dev/null || true; }
  trap cleanup_lock EXIT INT TERM
  echo "[local-macos] running locked npm ci"
  node "${script_dir}/run-with-timeout.mjs" \
    --timeout-ms "${SITES_INSTALL_TIMEOUT_MS:-480000}" \
    --kill-after-ms "${SITES_INSTALL_KILL_AFTER_MS:-15000}" \
  ...
  echo "[local-macos] npm ci passed and vinext is available"
  exit 0
fi
