...
expect(adminContent.includes("db.batch([revisionInsert, currentUpsert])") && adminContent.includes("AdminContentConflict"), "Editorial fields retain atomic history and optimistic concurrency protections.");
expect(adminPanel.includes('type SectionId = "dashboard" | "homepage" | "media" | "podcast" | "press"') && adminPanel.includes("What do you want to change?"), "Admin remains organized around owner workflows rather than raw configuration.");
expect(adminPanel.includes("+ Add event or appearance") && adminPanel.includes("Show on site") && adminPanel.includes("Draft only") && adminPanel.includes("Update live site"), "Admin supports clear draft/published appearance management.");
expect(adminPanel.includes("Protected for safety") && adminPanel.includes("Scientific claims, medical content"), "Admin clearly keeps scientific and medical content outside owner self-publishing.");
